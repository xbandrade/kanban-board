...
class CustomChoiceBox(ctk.CTkToplevel):
    def __init__(self, title, message, editable, items, *args, **kwargs):
        ...
        self.optionmenu_var = ctk.StringVar(
            value='No boards' if not self.menu_state else self.items[0]
        )
    ...
    def _create_widgets(self):
        ...
        ok_button = ctk.CTkButton(
            ...
            state=(['disabled', 'normal'][self.menu_state]
                   if not self.editable else 'normal')
        )
    ...
    def get_input(self):
        ...
        return (self.combobox_var.get()
                if self.editable else self.optionmenu_var.get())
...
class CustomConfirmationBox(ctk.CTkToplevel):
    ...
    def _create_widgets(self):
        ...
        self.label = ctk.CTkLabel(self, text=self.message)
        self.label.grid(
            row=0, column=0, columnspan=2, padx=20, pady=(20, 0), sticky='ew'
        )
